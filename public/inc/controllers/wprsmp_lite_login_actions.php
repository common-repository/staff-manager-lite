...
class LiteFrontDashBoardAction {
	...
	public static function clock_actions() {
		...
		if ( isset ( $_POST['timezone'] ) && isset ( $_POST['value'] ) ) {
			...
			if ( $value == 'office-in' ) {
				$shift_data   = WPRSMPLiteHelperClass::get_staff_shift( get_current_user_id() );
				...
				$data = array(
					...
					'name'         => WPRSMPLiteHelperClass::get_current_user_data( get_current_user_id(), 'fullname' ),
					'email'        => WPRSMPLiteHelperClass::get_current_user_data( get_current_user_id(), 'user_email' ),
					...
					'location'     => WPRSMPLiteHelperClass::get_user_location( $_SERVER['REMOTE_ADDR'] ),
				);
				...
				if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
					$message = esc_html__( 'Your Office In Time is ', 'staff-manger-lite' ).' '.esc_html( date( WPRSMPLiteHelperClass::get_time_format(), strtotime( $current_time ) ) );
					$status  = 'success';

					$save_settings  = get_option( 'wprsmp_settings_data' );
					if ( isset ( $save_settings['shoot_mail'] ) && $save_settings['shoot_mail'] == 'Yes' ) {
						WPRSMPLiteHelperClass::wprsmp_shoot_mail_staff_details( get_current_user_id(), $current_time, '', WPRSMPLiteHelperClass::get_user_location( $_SERVER['REMOTE_ADDR'] ), $_SERVER['REMOTE_ADDR'] );
					}
				...
				if ( ! empty ( $attendences ) ) {
					foreach ( $attendences as $key => $attendence ) {
						if ( $attendence['date'] == $current_date && $attendence['staff_id'] == $staff_id && ! empty ( $attendence['office_in'] ) ) {
							...
							if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
								$message = esc_html__( 'Your Lunch In Time is ', 'staff-manger-lite' ).' '.esc_html( date( WPRSMPLiteHelperClass::get_time_format(), strtotime( $current_time ) ) );
								$status  = 'success';
							} else {
								$message = esc_html__( 'Something went wrong.!', 'staff-manger-lite' );
								$status  = 'error';
				...
				if ( ! empty ( $attendences ) ) {
					foreach ( $attendences as $key => $attendence ) {
						if ( $attendence['date'] == $current_date && $attendence['staff_id'] == $staff_id && ! empty ( $attendence['office_in'] ) && ! empty ( $attendence['lunch_in'] ) && empty ( $attendence['lunch_out'] ) ) {
							...
							if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
								$message = esc_html__( 'Your Lunch Out Time is ', 'staff-manger-lite' ).' '.esc_html( date( WPRSMPLiteHelperClass::get_time_format(), strtotime( $current_time ) ) );
								$status  = 'success';
							} else {
								$message = esc_html__( 'Something went wrong.!', 'staff-manger-lite' );
								$status  = 'error';
					...
					foreach ( $attendences as $key => $attendence ) {
						if ( $attendence['date'] == $current_date && $attendence['staff_id'] == $staff_id && ! empty ( $attendence['office_in'] ) && empty ( $attendence['office_out'] ) ) {
							...
							if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
								...
								$total_working_duration = WPRSMPLiteHelperClass::get_time_difference( $attendence['office_in'], $attendence[$key]['office_in'] );
								...
								if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
									$message = esc_html__( 'Your Office Out Time is ', 'staff-manger-lite' ).' '.esc_html( date( WPRSMPLiteHelperClass::get_time_format(), strtotime( $attendence[$key]['office_out'] ) ) );
									$status  = 'success';
									WPRSMPLiteHelperClass::wprsmp_shoot_mail_staff_details( get_current_user_id(), $attendence['office_in'], $current_time, WPRSMPLiteHelperClass::get_user_location( $_SERVER['REMOTE_ADDR'] ), $_SERVER['REMOTE_ADDR'] );
								} else {
									$message = esc_html__( 'Something went wrong.!', 'staff-manger-lite' );
									$status  = 'error';
								}
			...
			wp_send_json( esc_html__( 'Something went wrong.!', 'staff-manger-lite' ) );
		}
		wp_die();
	}
			...
			if ( ! empty ( $attendences ) ) {
				foreach ( $attendences as $key => $attendence ) {
					if ( $attendence['date'] == $current_date && $attendence['staff_id'] == $staff_id && ! empty ( $attendence['office_in'] ) ) {
						...
						if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
							wp_send_json( esc_html__( 'Late Reason Updated.!', 'staff-manger-lite' ) );
						} else {
							wp_send_json( esc_html__( 'Something went wrong.!', 'staff-manger-lite' ) );
						}
					}
			...
			if ( ! empty ( $attendences ) ) {
				foreach ( $attendences as $key => $attendence ) {
					if ( $attendence['date'] == $current_date && $attendence['staff_id'] == $staff_id && ! empty ( $attendence['office_in'] ) ) {
						...
						if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
							wp_send_json( esc_html__( 'Daily Report Submitted.!', 'staff-manger-lite' ) );
						} else {
							wp_send_json( esc_html__( 'Something went wrong.!', 'staff-manger-lite' ) );
						}
					}
