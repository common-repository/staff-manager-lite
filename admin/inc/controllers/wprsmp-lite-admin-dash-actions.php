	...
	public static function clock_actions() {
        ...
        if ( isset( $_POST['timezone'] ) && isset( $_POST['value'] ) ) {
            ...
            if ( $value == 'office-in' ) {
                ...
                $shift_data = WPRSMPLiteHelperClass::get_staff_shift($staff_key);
                ...
                $data = array(
                    'staff_id'     => $staff_key,
                    'name'         => WPRSMPLiteHelperClass::get_current_user_data( $staff_key, 'fullname' ),
                    'email'        => WPRSMPLiteHelperClass::get_current_user_data( $staff_key, 'user_email' ),
                ...
                if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {           
                    $message = __( 'Staff Office In Time is ', 'staff-manger-lite' ).' '.$current_time;
                    $status  = 'success';
                } else {
					...
					foreach ( $attendences as $key => $attendence ) {
						if ( $attendence['date'] == $current_date && $attendence['staff_id'] == $staff_key && ! empty ( $attendence['office_in'] ) && empty ( $attendence['office_out'] ) ) {
							...
							if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
								...
								if ( update_option( 'wprsmp_staff_attendence_data', $attendences ) ) {
									$message = __( 'Staff Office Out Time is ', 'staff-manger-lite' ).' '.$attendence[$key]['office_in'];
									$status  = 'success';
								} else {
									$message = __( 'Something went wrong.!', 'staff-manger-lite' );
									$status  = 'error';
                            ...
                            $message = __( 'Staff already logged out.!', 'staff-manger-lite' );
							$status  = 'error';
                        }
					}
				}
