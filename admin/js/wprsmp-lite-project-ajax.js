...
jQuery(document).ready(function () {
    ...
    jQuery(document).on('click', '#add_project_btn', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_add_project_ajax',
                name: name,
                desc: desc,
                member: member,
                tags: tags,
    ...
    jQuery(document).on( 'click', '.project-edit-a', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_edit_project_ajax',
                key: key,
                nounce: nounce,
            },
            success: function ( response ) {
    ...
    jQuery(document).on('click', '#edit_project_btn', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_update_project_ajax',
                name: name,
                key: key,
                desc: desc,
                member: member,
    ...
    jQuery(document).on( 'click', '.project-delete-a', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_delete_project_ajax',
                key: key,
                nounce: nounce,
            },
            success: function ( response ) {
    ...
    jQuery(document).on( 'click', '.project-view-a', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_view_all_tasks_ajax',
                key: key,
                nounce: nounce,
            },
            success: function ( response ) {
    ...
    jQuery(document).on('click', '#edit_task_btn', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_update_task_ajax',
                task_key: task_key,
                proj_key: proj_key,
                name: name,
                desc: desc,
    ...
    jQuery(document).on( 'click', '.task__delete-btn', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_delete_task_ajax',
                proj_key: proj_key,
                task_key: task_key,
                nounce: nounce,
            },
    ...
    jQuery(document).on( 'click', '#upload-btn-wprsmp', function (e) {
        ...
        .on('select', function(e){
            ...
            for (var i = 0; i < uploaded_image.length; i++) {
               jQuery('#myplugin-placeholder_wprsmp').append(
                    '<div class="myplugin-image-previeww"><img src="' + 
                    uploaded_image[i].attributes.url + '" >'
                    );
                jQuery('#myplugin-placeholder_wprsmp').append(
                    '<input id="image_url_wprsmp-'+ i +'" type="hidden" name="myplugin_attachment_url"  value="' + uploaded_image[i].attributes.url + '"></div>' 
                    );
            }
            jQuery( '#image_length_wprsmp' ).val( uploaded_image.length );
            jQuery( 'body' ).addClass( 'modal-open' );
        });
    });
    ...
    jQuery(document).on( 'click', '#add_comment_task_btn', function () {
        ...
        var image_length_wprsmp = jQuery('input#image_length_wprsmp').val();
        var media = [];
        for ( var i = 0; i < image_length_wprsmp; i++ ) {
          media[i] = jQuery('input#image_url_wprsmp-'+ i).val();
        }
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_add_comment_ajax',
                proj_key: proj_key,
                task_key: task_key,
                comment: comment,
                media: media,
            ...
            success: function ( response ) {
                if ( response.status == 'error' ) {
                    ...
                    jQuery( '#myplugin-placeholder_wprsmp' ).empty();
                }
            }
        });
    });
    ...
    jQuery(document).on( 'click', '.comment-edit-btn', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_edit_comment_ajax',
                proj_key: proj_key,
                task_key: task_key,
                ...
                if ( response == 'Something went wrong.!' ) {
                    ...
                    if ( media_files != undefined || media_files.length != 0 ) {
                        ...
                        for ( var i = 0; i < media_length; i++ ) {
                            ...
                            jQuery('#myplugin-placeholder_wprsmp').append(
                                 '<input id="image_url_wprsmp-'+ i +'" type="hidden" name="myplugin_attachment_url"  value="' + response.media[i] + '"></div>' 
                            );
                        }
                    }
                }
    ...
    jQuery(document).on( 'click', '#edit_comment_task_btn', function () {
        ...
        jQuery( '#myplugin-placeholder_wprsmp input' ).each(function() {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_update_comment_ajax',
                proj_key: proj_key,
                task_key: task_key,
    ...
    jQuery(document).on( 'click', '.comment-delete-btn', function () {
        ...
        jQuery.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wprsmp_delete_comment_ajax',
                proj_key: proj_key,
                task_key: task_key,
                comment_key: comment_key,
                nounce: nounce,
